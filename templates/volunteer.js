"use strict";
    ...
    function formValidation() {
        ...
        var birthd = document.registration.birthd.value;
        ...
        if (birthd == "") {
            alert('Enter the birth!');

            document.registration.birthd.focus();
        ...
        //age, address, city, postal, province, province2, country
            if (fname != '' && lname != '' && phone != '' && email != '' && age != '' && birthd != '' && address != '' && city != '' && postal != '' && province2 != '' && country != '') // condition for check mandatory all fields

            {
                let confirmation = "Once you submit this form, you can't go back \nAre you sure you want to leave this page?";
                if (confirm(confirmation) == true) {
                    const dict_values = {fname, lname, phone, email, age, birthd, address, city, postal, province2, country} //Pass the javascript variables to a dictionary.
                    ...
                    fetch(urlPath)
                        .then(response => {
                            ...
                            if(response.status == 200) {
                                window.location.href = "/volunteerForm2";
                            }
                        })
                        .catch(err => {
                            console.log(err);
